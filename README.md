...
- **In Kind Act Edit/Delete Section** - Need to figure out how to make the user who creates the kind act, the only one who can edit and delete it. Currently, I haven't figured out how to do it for the deadline of class; but will keep researching and try and figure it out, and then update the code. (Solution has something to do with having to create a middleware function that checks the user ID and then create an if else statement in the route that calls the function.) 
